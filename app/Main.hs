...
import XMonad.Hooks.SetWMName as SetWMName
    ...
    $ desktopConfig
      { layoutHook =
          minimize
            ...
            Mirror
            $ G.SplitGrid G.B 1 1 (3 / 5) (9 / 16) stepSize
      , modMask = mod4Mask
      , keys = myKeys
      , normalBorderColor = "#222222"
      , focusedBorderColor = "#FFB53A"
      , borderWidth = 3
      , startupHook = setFullscreenSupported *> SetWMName.setWMName "LG3D"
...
myKeys conf@XConfig{XMonad.modMask = modMask} =
  M.fromList
    ...
    ,
      ( (modMask, xK_q)
      , spawn
          "if type xmonad; then xmonad --recompile && xmonad --restart && zenity --info --text='xmonad restarted'; else zenity --info --text='xmonad not in \\$PATH: \"$PATH\"'; fi" -- %! Restart xmonad
